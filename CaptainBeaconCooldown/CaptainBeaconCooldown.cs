...
namespace CaptainBeaconCooldown
{
    ...
    public class CaptainBeaconCooldown : BaseUnityPlugin
    {
        ...
        public const float DEFAULT_HEALING_BEACON_COOLDOWN = 40.0f;
        public const float DEFAULT_SHOCKING_BEACON_COOLDOWN = 40.0f;
        public const float DEFAULT_EQUIPMENT_BEACON_COOLDOWN = 60.0f;
        public const float DEFAULT_HACKING_BEACON_COOLDOWN = 180.0f;

        public float[] beaconCooldowns;
        public string[] beaconNameTokens = new string[] { "CAPTAIN_SUPPLY_HEAL_NAME", "CAPTAIN_SUPPLY_SHOCKING_NAME", "CAPTAIN_SUPPLY_EQUIPMENT_RESTOCK_NAME", "CAPTAIN_SUPPLY_HACKING_NAME" };
        ...
        private void OnRoR2SkillsCaptainSupplyDropSkillDefIsReady(On.RoR2.CaptainSupplyDropController.orig_FixedUpdate orig, RoR2.CaptainSupplyDropController self)
        {
            ...
            {
                leftBeaconStopwatch += Time.deltaTime; // increment stopwatch for beacon cooldowns
                Log.Info($"Value of Time.deltaTime: {Time.deltaTime}");
            }
            ...
            {
                rightBeaconStopwatch += Time.deltaTime; // increment stopwatch for beacon cooldowns
            }
            ...
            {
                Log.Info($"==================== Starting to print all values of supplydrop ====================");
                ...
                Log.Info("Printing variable named: 'self.supplyDrop1Skill.beginSkillCooldownOnSkillEnd'");
        ...
        {
            On.RoR2.CaptainSupplyDropController.FixedUpdate += OnRoR2SkillsCaptainSupplyDropSkillDefIsReady;
        }
        ...
        public void Awake()
        {
            ...
            // Config Setup
            HealingBeaconCD = Config.Bind<float>("Beacon Cooldowns", "Healing Beacon Cooldown", DEFAULT_HEALING_BEACON_COOLDOWN, "Captain's Healing Beacon Cooldown in seconds. Default: " + DEFAULT_HEALING_BEACON_COOLDOWN);
            ShockingBeaconCD = Config.Bind<float>("Beacon Cooldowns", "Shocking Beacon Cooldown", DEFAULT_SHOCKING_BEACON_COOLDOWN, "Captain's Shocking Beacon Cooldown in seconds. Default: " + DEFAULT_SHOCKING_BEACON_COOLDOWN);
            EquipmentBeaconCD = Config.Bind<float>("Beacon Cooldowns", "Equipment Beacon Cooldown", DEFAULT_EQUIPMENT_BEACON_COOLDOWN, "Captain's Equipment Beacon Cooldown in seconds. Default: " + DEFAULT_EQUIPMENT_BEACON_COOLDOWN);
            HackingBeaconCD = Config.Bind<float>("Beacon Cooldowns", "Hacking Beacon Cooldown", DEFAULT_HACKING_BEACON_COOLDOWN, "Captain's Hacking Beacon Cooldown in seconds. Default: " + DEFAULT_HACKING_BEACON_COOLDOWN);
            EnableDebug = Config.Bind<bool>("Enable Debug", "Enable Debugging Keybind(s)", false, "Enable debug keybinds (Like L to print values to console)");

            // Populate Beacon Cooldown Array
            this.beaconCooldowns = new float[] { HealingBeaconCD.Value, ShockingBeaconCD.Value, EquipmentBeaconCD.Value, HackingBeaconCD.Value };
